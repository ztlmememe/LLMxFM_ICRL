import gc
import torch
from chemistry.SC_scripts.MOL_trainer_pca_icl_baseline import run as run_base_batch
import argparse

def parse_args():
    parser = argparse.ArgumentParser(description="Run MOL experiments.")
    parser.add_argument("--experiment_type", type=str, choices=["pca_icl", "icl", "pca"], default="pca_icl", help="Type of experiment to run.")
    ...
    parser.add_argument("--task_name", type=str, choices=["ESOL","Caco2_wang","Solubility_AqSolDB","LD50_Zhu","Clearance_Hepatocyte_AZ","HydrationFreeEnergy_FreeSolv","Lipophilicity_AstraZeneca"], default="ESOL", help="Task name.")
...
if __name__ == "__main__":
    ...
    if experiment_type == "pca_icl":
        use_icl_string = True
        use_rep = True
        insert_rep_as_string = True
        experiment_name = f"{experiment_name}_pca_icl"

    elif experiment_type == "icl":
        use_icl_string = True
        use_rep = False
        insert_rep_as_string = False
        experiment_name = f"{experiment_name}_icl"

    elif experiment_type == "pca":
        use_icl_string = False
        use_rep = True
        insert_rep_as_string = True
        experiment_name = f"{experiment_name}_pca"
    ...
    if task_name == "Caco2_wang":
        predicted_property = "Permeation rate through Caco-2 cells"
        # predicted_property = "Permeation rate"
    
    elif task_name == "Solubility_AqSolDB":
        ...
        predicted_property = "Acute toxicity"
        
    elif task_name == "Clearance_Hepatocyte_AZ":
        predicted_property = "Activity of clearance"

    elif task_name == "HydrationFreeEnergy_FreeSolv":
        predicted_property = "Activity of hydration free energy"
    
    elif task_name == "Lipophilicity_AstraZeneca":
        predicted_property = "Activity of lipophilicity"
