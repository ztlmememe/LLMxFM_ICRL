...
# data.harmonize_affinities(mode = 'mean')
    ...
    if load_pairs:
        ...
        for i, test_input in tqdm(enumerate(test_inputs), total=len(test_inputs), desc="train rep"):

            if len(test_rep_pairs) >= max_test_reps:
                break
            
            # Process the input (SMILES, optionally protein sequence)
            if task_type == "DTI":
                test_smile, test_protein = test_input

            # Check and retrieve or compute molecular representation
            if test_smile not in mol_rep_cache:
                # Generate and cache molecular representation if not already available
                unimol_rep = mol_fm.get_unimol_rep_tensor(test_smile)
                mol_rep_cache[test_smile] = unimol_rep
            else:
                unimol_rep = mol_rep_cache[test_smile]

            # Check and retrieve or compute protein representation
            if test_protein not in protein_rep_cache:
                # Generate and cache protein representation if not already available
                protein_rep = protein_fm.get_esm_rep_tensor(test_protein, single_input_batch=True) if task_type == "DTI" else None
                protein_rep_cache[test_protein] = protein_rep
            else:
                protein_rep = protein_rep_cache[test_protein]

            # Store the pair of (unimol_rep, protein_rep)
            test_rep_pairs.append((unimol_rep, protein_rep))

        print("test_inputs.shape: ", test_inputs.shape)
        print("len(test_rep_pairs): ", len(test_rep_pairs))
        
        # with open(f'{base_data_path}/{task_type}_{task_name}_test_mol_protein_pairs.pkl', 'wb') as f:
        #     pickle.dump(test_rep_pairs, f)
    ...
    with torch.no_grad():
        ...
        if use_rep:
            ...
            if len(example_smiles) > 0:
                ...
                mol_reps = mol_fm.get_unimol_rep_tensor(example_smiles)

                if rep_dim_reduction == "PCA":
                    # print("example mol_reps: ", mol_reps)
                    # print("example mol_reps shape: ", mol_reps.shape)
                    example_mol_reduced_reps = mol_pca.transform(mol_reps.cpu().numpy())
                    ...
                    mol_reps = torch.tensor(example_mol_reduced_reps).to(mol_reps.device)
        ...
        for example_i, (example_feature, unimol_rep, protein_rep, label) in enumerate(zip(example_features, mol_reps, protein_reps, example_labels)):
                    ...
                    if use_rep:
                        ...
                        if use_icl_string:
                            ...
                            if insert_rep_as_string:
                                ...
                                str_unimol_rep = fixed_sig_fig_str_tensor(unimol_rep, text_rep_sig_figs)
                                # print("str_unimol_rep A: ", str_unimol_rep)
                                llm_prompt_text_icl_template.append(str_unimol_rep)
                                if load_HF_llm:
                                    user_prompt_embeddings_template = concatenate_text_to_embeddings(
                                        user_prompt_embeddings_template, str_unimol_rep, model, tokenizer
                                    )
                            else:
                                if load_HF_llm:
                            ...
                            if insert_rep_as_string:
                                # concatenate rep as string to text
                                # print("protein_rep C: ", protein_rep)
                                str_protein_rep = fixed_sig_fig_str_tensor(protein_rep, text_rep_sig_figs)
                                # print("str_protein_rep C: ", str_protein_rep)
            ...
            if task_type == "DTI":
                if use_rep:
                    ...
                    if use_icl_string:
                        ...
                        unimol_rep = mol_fm.get_unimol_rep_tensor(test_smile)[0]
                        ...
                        if insert_rep_as_string:
                                ...
                                str_unimol_rep = fixed_sig_fig_str_tensor(unimol_rep, text_rep_sig_figs)
                                user_prompt = f"{user_prompt}{str_unimol_rep}"
                                if load_HF_llm:
                                    model_input_embeddings = concatenate_text_to_embeddings(
                                        model_input_embeddings, str_unimol_rep, model, tokenizer
                                    )
                        else:
                                if load_HF_llm:
                        ...
                        protein_rep = protein_fm.get_esm_rep_tensor(test_protein)[0]
                        if rep_dim_reduction == "PCA":
                            # print("test protein_reps: ", protein_rep)
                            # print("test protein_reps shape: ", protein_rep.shape)
                            if len(protein_rep.shape) == 1:
                                protein_rep_np = protein_rep.unsqueeze(0).cpu().numpy()
                            else:
                                protein_rep_np = protein_rep.cpu().numpy()
                            test_protein_reduced_rep = pro_pca.transform(protein_rep_np)
                            # print("test test_protein_reduced_rep: ", test_protein_reduced_rep)
                            # print("test test_protein_reduced_rep.shape: ", test_protein_reduced_rep.shape)
                            if len(protein_rep.shape) == 1:
                                protein_rep = torch.tensor(test_protein_reduced_rep[0]).to(protein_rep.device)
                            else:
                                protein_rep = torch.tensor(test_protein_reduced_rep).to(protein_rep.device)


                        if insert_rep_as_string:
                            # concatenate rep as string to text
                            # print("protein_rep E: ", protein_rep)
                            str_protein_rep = fixed_sig_fig_str_tensor(protein_rep, text_rep_sig_figs)
                            # print("str_protein_rep E: ", str_protein_rep)
                            # rep_string = f"{str_unimol_rep}"
