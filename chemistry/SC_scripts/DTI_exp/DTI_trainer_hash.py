...
# data.harmonize_affinities(mode = 'mean')
    ...
    with torch.no_grad():
        ...
        if use_rep:
            ...
            if len(example_smiles) > 0:
                ...
                for j, example_smile in enumerate(example_smiles):
                    ...
                    unimol_rep = string_to_tensor(example_smile, shape=(1, mol_dim_size)).to(
                        model.model.embed_tokens.weight.device
                    )
        ...
        for example_i, (example_feature, mol_rep, protein_rep, label) in enumerate(zip(example_features, mol_reps, protein_reps, example_labels)):
            ...
            if task_type == "DTI":
                if use_rep:
                    ...
                    if use_icl_string:
                        ...
                        unimol_rep = string_to_tensor(test_smile, shape=(1, mol_dim_size)).to(
                        model.model.embed_tokens.weight.device 
                    )
                        ...
                        test_mol_reduced_rep = torch.squeeze(mapper_output, dim=0)
