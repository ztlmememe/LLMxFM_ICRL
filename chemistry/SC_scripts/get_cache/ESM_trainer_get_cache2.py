...
def save_cache_safely(cache_path, data_map):
    ...
    #     print("Successfully saved aaseq_to_rep_store to cache.")
...
def parse_args():
    ...
    parser.add_argument('--task_name', type=str, choices=["Stability", "Fluorescence", "Beta_Lactamase", "PPI_Affinity"],
                         help='Name of the dataset task (e.g., Stability)')
...
def extract_protein_representations(protein_fm, protein_seqs, batch_size=512, device="cuda",cache_path=""):
    """
    Extract protein representations using the model's default method (e.g., CLS token from last layer).
    """
    if not protein_seqs:
        print("no protein sequences provided for extraction.")
        return
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader, desc="Extracting protein representations"):
            ...
            print(tokenizer_to_use.convert_ids_to_tokens(protein_input_ids['input_ids'][0]) )
...
def run(
    ...
    all_sequences = set()
    available_tasks = ["Fluorescence"] #, "Beta_Lactamase", "PPI_Affinity"]
    
    tasks_to_run = available_tasks
    cache_path = '/mnt/ssd/ztl/LLMxFM/chemistry/datasets/prot_bert_aaseq_to_rep_store_layer_5.pkl'
    for task in tasks_to_run:
        ...
        protein_fm = prot_bert_model(model_name="Rostlab/prot_bert", 
        ...
        # print cache -5 item
        # print("cache keys: ", list(existing_sequences)[:-5])

        # sequences = [preprocess_protein_sequence(seq) for seq in sequences]
        all_sequences = set([preprocess_protein_sequence(seq) for seq in all_sequences])
        ...
        if sequences_to_compute:
            ...
            cache_path = cache_path = '/mnt/ssd/ztl/LLMxFM/chemistry/datasets/prot_bert_aaseq_to_rep_store_cls.pkl'
            ...
            print("Cache already contains all sequences. No extraction needed.")
...
# CUDA_VISIBLE_DEVICES=0 python -m chemistry.SC_scripts.ESM_trainer_get_cache2 --base_data_path /mnt/ssd/ztl/LLMxFM/chemistry/datasets --task_name Fluorescence
