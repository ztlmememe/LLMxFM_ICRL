...
def get_llm_text_and_projected_features(
    ...
    target_dtype,
    max_length=128,
    sampling_strategy="random"  # "random", "fixed", "front", "middle", "back"
):
    """
    Computes LLM hidden states for both text-based input (SMILES) and projected input (dense vectors),
    matching them at the same randomly sampled hidden layer.
    ...
    print(f"Number of transformer layers in LLM: {num_transformer_layers}") 
    # Number of transformer layers in LLM: 32
    # === Determine Sampling Layer (shared between text and proj) ===
    ...
    elif sampling_strategy == "random_limit":
        third = num_transformer_layers // 3
        segment = random.choice(["front", "middle", "back"])
        if segment == "front":
            actual_layer_index = random.randint(1, 5)
        elif segment == "middle":
            actual_layer_index = random.randint(third - 3, third + 3)
        else:
            actual_layer_index = random.randint(num_transformer_layers - 4, num_transformer_layers)

    elif sampling_strategy == "front":
        # actual_layer_index = random.randint(1, max(1, num_transformer_layers // 3))
        actual_layer_index = random.randint(1, 5)
    elif sampling_strategy == "middle":
        # actual_layer_index = random.randint(num_transformer_layers // 3 + 1, 2 * num_transformer_layers // 3)
        actual_layer_index = random.randint(num_transformer_layers // 3 - 3, 2 * num_transformer_layers // 3 + 3)
    elif sampling_strategy == "back":
        # actual_layer_index = random.randint(2 * num_transformer_layers // 3 + 1, num_transformer_layers)
        actual_layer_index = random.randint(num_transformer_layers - 4, num_transformer_layers)
    else:
    ...
    # print("proj_hidden_layer shape:", proj_hidden_layer.shape)
    # proj_hidden_layer shape: torch.Size([16, 1, 4096]) 
    proj_token_features = proj_hidden_layer[:, -1, :].to(dtype=target_dtype)

    print(f"Using LLM hidden layer {actual_layer_index}/{num_transformer_layers} for contrastive loss.")
    print(f"Gradient flow test: proj={proj_token_features.requires_grad}, text={text_last_token_features.requires_grad}")
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, smiles_batch in progress_bar_train:
            ...
            target_text_features, projected_features = get_llm_text_and_projected_features(
                ...
                sampling_strategy="fixed" # Use random_limit for validation
            )
